...
export const staffDocuments = {
  getAll: (staffId) => request(`/staff/${staffId}/documents`),
  upload: (staffId, formData) => {
    const token = localStorage.getItem("auth_token");
    return fetch(`${getApiBaseUrl()}/staff/${staffId}/documents`, {
      method: "POST",
  ...
  getDownloadUrl: (staffId, docId) => `${getApiBaseUrl()}/staff/${staffId}/documents/${docId}/download`,
  delete: (staffId, docId) => request(`/staff/${staffId}/documents/${docId}`, { method: "DELETE" }),
};
...
export const destinations = {
  ...
  uploadImage: (file) => {
    ...
    return fetch(`${getApiBaseUrl()}/destinations/upload-image`, {
      method: "POST",
      headers: token ? { Authorization: `Bearer ${token}` } : {},
      body: formData,
    }).then(async (res) => {
...
export const staffSchedules = {
  get: (staffId) => request(`/staff/${staffId}/schedules`),
  save: (staffId, schedules) => request(`/staff/${staffId}/schedules`, { method: "PUT", body: { schedules } }),
  delete: (scheduleId) => request(`/staff/schedules/${scheduleId}`, { method: "DELETE" }),
};

export const staffExceptions = {
  getAll: (staffId) => request(`/staff/${staffId}/exceptions`),
  create: (staffId, formData) => {
    const token = localStorage.getItem("auth_token");
    return fetch(`${getApiBaseUrl()}/staff/${staffId}/exceptions`, {
      method: "POST",
      headers: token ? { Authorization: `Bearer ${token}` } : {},
      body: formData,
    }).then(async (res) => {
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Error creando excepción");
      return data;
    });
  },
  getDownloadUrl: (exceptionId) => `${getApiBaseUrl()}/staff/exceptions/${exceptionId}/download`,
  delete: (exceptionId) => request(`/staff/exceptions/${exceptionId}`, { method: "DELETE" }),
};
